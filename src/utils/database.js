...
export function addBlacklistNumber(groupId, number) {
  ...
  if (!data[groupId].numbers.includes(number)) {
    data[groupId].numbers.push(number);
  }

  writeJSON(BLACKLIST_FILE, data, {});
}

export function removeBlacklistNumber(groupId, number) {
  const data = readJSON(BLACKLIST_FILE, {});
  ...
  const index = data[groupId].numbers.indexOf(number);

  if (index === -1) {
    return false;
  }

  data[groupId].numbers.splice(index, 1);
...
export function listBlacklistNumbers(groupId) {
  const data = readJSON(BLACKLIST_FILE, {});

  return data[groupId]?.numbers || [];
}

export function isBlacklistActive(groupId) {
  const data = readJSON(BLACKLIST_FILE, {});

  return data[groupId]?.active === true;
}

export function setBlacklistActive(groupId, active) {
  const data = readJSON(BLACKLIST_FILE, {});
...
const MESSAGE_RANK_FILE = "message-rank";

export function addMessageRank(groupId, memberId) {
  const data = readJSON(MESSAGE_RANK_FILE, {});

  if (!data[groupId]) {
    data[groupId] = {};
  }

  data[groupId][memberId] = (data[groupId][memberId] || 0) + 1;

  writeJSON(MESSAGE_RANK_FILE, data, {});
}

export function getMessageRank(groupId, limit = 10) {
  const data = readJSON(MESSAGE_RANK_FILE, {});
...
export function getMemberMessageRank(groupId, memberId) {
  const data = readJSON(MESSAGE_RANK_FILE, {});
  return data[groupId]?.[memberId] || 0;
}

export function resetMessageRank(groupId) {
  const data = readJSON(MESSAGE_RANK_FILE, {});
