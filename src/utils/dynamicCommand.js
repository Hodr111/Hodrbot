...
export async function dynamicCommand(paramsHandler, startProcess) {
  ...
  // Controle de aluguel por grupo.
  // O comando "alugar" continua disponível para o dono do bot
  // mesmo quando o aluguel do grupo estiver vencido.
  if (
    activeGroup &&
    !isUnlimited(remoteJid) &&
    !isRentalActive(remoteJid) &&
    getRental(remoteJid) &&
    commandName !== "alugar"
  ) {
    await sendReply(
...
O período de aluguel deste grupo chegou ao fim e, por isso, os comandos do bot foram temporariamente desativados.
...
Quer continuar usando o bot?

Entre em contato com o responsável pela renovação:

👉 https://wa.me/+5515981852172?text=Opa%20G%20vim%20renovar%20nosso%20aluguel%20%F0%9F%87%AF%F0%9F%87%B5
...
Obrigado por utilizar nossos serviços! 🤍`
  ...
  try {
    ...
    if (result?.text) {
      await paramsHandler.sendReply(result.text, result.mentions);
    }
  } catch (error) {
