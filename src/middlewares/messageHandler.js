...
export async function messageHandler(socket, webMessage) {
  try {
    ...
    // Ranking ativo: conta mensagens dos membros.
    addMessageRank(remoteJid, userLid);

    // O dono precisa poder executar comandos do bot.
    // Apenas mensagens do próprio bot são ignoradas aqui.
    if (userLid === BOT_LID) {
      return;
    }
